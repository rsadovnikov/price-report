  ...
  (function() {
    ...
    // «Отслеживается», пока не уйдёшь с вкладки. Сбрасывается при входе на «Активные».
    ...
    // публикации»), остаток — «Новые конкуренты» из подборки. Каунтер таба «Активные» = остаток.
    ...
    /* Описание базового объекта — ОДНО место на страницу.
       До 2026-09-08 его выводили дважды: `propBaseObject` собирал из параметров и
       «Моих объявлений» и клал в `.desc-title`, а предустановка фильтров читала уже
       переписанную им разметку. Совпадало только потому, что блоки идут в этом порядке;
       читать разметку, которую сам же и заполнил, значит зависеть от порядка в файле.
       Раскладке апдейтов описание нужно РАНЬШЕ, чем `propBaseObject` успевает отработать,
       поэтому источник тут, а не в DOM. */
    function baseDesc() {
      var my = (typeof MY_LISTINGS !== 'undefined' && MY_LISTINGS[0]) || {};
      return REPORT_PARAMS.get('desc') || my.desc || '';
    }
    ...
    (function markUpdates() {
      ...
      /* 3) Остаток — «Новые конкуренты»: активные НЕ отслеживаемые из подборки.
         ...
         🔴 **Кандидат обязан быть виден в «Активных» без единой правки фильтров.**
         До 2026-09-08 отбор шёл по всей базе, а «Активные» показывают выдачу с
         предустановкой (комнаты и площадь из объекта, метро 15, радиус). Плашка
         обещала шесть новых, агент нажимал «Посмотреть» и не находил ни одного:
         помеченные объекты в предустановку не попадали. Дефект 5 отчёта аудита;
         в приложении тот же класс чинили 2026-09-01 (`AppFilters.inPreset`).
         ⚠️ Здесь `passes(…, preset(desc))`, а НЕ `inPreset`: тот добавляет
         `AppPreset.comparable`, которого вебовая выдача намеренно не применяет —
         с ним апартаменты пропали бы из «Активных» (23 строки против нуля).
         Правило простое: спрашиваем ровно тем же, чем строится выдача (renderTableB
         фильтрует `AppFilters.passes(x.d, filterState)`), и никаким другим. */
      var предустановка = AppFilters.preset(baseDesc());
      var pool = [];
      for (var n = 0; n < ALL_COMPETITORS.length; n++) {
        if (checkedIds.has(n) || removedIds.has(n)) continue;
        if (!AppFilters.passes(ALL_COMPETITORS[n], предустановка)) continue;
        pool.push(n);
      }
    ...
    // Сколько новых конкурентов ещё в «Активных» (не ушли в отслеживаемые) — каунтер таба.
    function newCount() {
      var c = 0;
    ...
    function makeRow(d, idx, isChecked) {
      // В «Активных» добавленные строки остаются залитыми (как hover) — маркер «уже добавлен».
      ...
      //   «Отслеживаемые» → снятие с публикации / изменение цены отслеживаемого;
      //   «Активные» → «Новый конкурент».
      // Серый (.badge-removed) = просто статус архивного объявления (фильтр «Архивные»
      //   в «Активных») — это не апдейт, а признак самого объекта.
      var updateBadge = '';
      if (activeTab === 'in-report') {
        ...
        /* Направление словом плюс сумма (правка Романа 2026-09-08). Знак числом не
           пишем: направление уже сказано словом. Без дельты остаётся нейтральное
           «Цена изменилась» — направления не знаем.
           Ту же подпись собирает `AppUpdates.priceLabel` для двух экранов приложения;
           сводить одним модулем нельзя (он уезжает сборкой в /app/), поэтому
           поверхности сверяются проверкой, а не копированием строки. */
        else if (priceUpdatedIds.has(idx)) {
          /* 🔴 Последнее изменение, а не `priceDelta`: та считается от СТАРТОВОЙ цены,
             то есть это сумма всех движений за срок размещения. В колонке цены она
             уместна — колонка так и подписана, «Разница со стартовой», — а бейдж
             сообщает свежую новость, и тотал за три месяца в нём врёт. */
          var последнее = d.priceHistory && d.priceHistory[0];
          var сумма = последнее ? String(последнее.delta || '').replace(/[−+-]/g, '').trim() : '';
          updateBadge = '<div class="label label-warning badge-update">'
            + (сумма
              ? 'Цена ' + (последнее.isUp ? 'увеличилась' : 'уменьшилась') + ' на ' + сумма + ' ₽'
              : 'Цена изменилась')
            + '</div>';
        }
      } else {
    ...
    function renderTableB() {
      ...
      if (activeTab === 'in-report') {
        ...
        // окно visibleCount применяем ПОСЛЕ сортировки (только в «Активных»), чтобы новые
        ...
        for (var j = 0; j < ALL_COMPETITORS.length; j++) {
          ...
          // «Активные» — только активные; «Архивные» — только снятые с публикации (removedIds).
      ...
      // вовсе. На «Отслеживаемых» фильтры не показываются, значит и не применяются:
      // там не выдача, а выбор агента.
      if (activeTab !== 'in-report') {
        items = items.filter(function (x) { return AppFilters.passes(x.d, filterState); });
      }

      // «Активные» — по близости к объекту пользователя: сверху то, с чем его реально
      ...
      // Баннер «Следим за изменениями» — только на вкладке «Отслеживаемые» при ≥1 объекте
      if (trackingBanner) {
        trackingBanner.style.display =
          (activeTab === 'in-report' && checkedIds.size > 0) ? 'flex' : 'none';
      }
      // Баннер «Постоянно следим за новыми» — только на вкладке «Активные»
      if (selectionBanner) {
     ...
     */
    /* Тот же `baseDesc()`, что у раскладки апдейтов: две предустановки, собранные из
       разных источников, — это и был дефект 5. */
    var filterState = AppFilters.preset(baseDesc());
    ...
    function ownerPanelReached() {
      ...
      // Меряем строку целиком, а не число внутри неё: у них общий низ, но строка честнее.
      var line = step.closest('.owner-step-title') || step;
    ...
    (function watchShelfLanding() {
      ...
      /* 🔴 Прокрутки и ресайза мало. Первый расчёт приходится на момент, когда фотографии
         ...
         Поэтому пересчитываем ещё дважды: по `load` (картинки доехали) и на любое
         изменение высоты ленты — она и двигает карточку отчёта. */
      window.addEventListener('load', onScroll);
      if (window.ResizeObserver) {
        var section = document.querySelector('.report-section');
        if (section) new ResizeObserver(onScroll).observe(section);
      }
    })();
    ...
    // Сколько активных конкурентов доступно в «Активных» (не в отчёте). Архив — отдельная
    ...
    // экспозиции). Баннер ценности + ТОТ ЖЕ фасетный ряд, что в «Активных» (общий `.filters`,
    ...
    tabArchive.addEventListener('click', function() {
      ...
      filtersEl.style.display = '';        // общий фасетный ряд — как в «Активных» (синхрон)
      resultsCounter.style.display = 'none';
    ...
    // Фильтр «Активные/Архивные» (чипы в «Активных») удалён — архив вынесен в отдельную вкладку.
    ...
    // «Отслеживаемые»: «Удалить» → строка схлопывается + снекбар с «Восстановить».
    // «Активные»: «Добавить» (синяя) ↔ «Отслеживается» — тоггл на месте, со снекбарами; строка остаётся.
  ...
  (function helpTrigger() {
    ...
    /* Первый слайд — по макету Figma 2690:129422: лейаут 'window', тело — живая вёрстка
       ...
       `autoplay: true` у слайда. Путь без `?v=`: версию ставит сборка (deploy-pages.sh),
       ролик меняется под тем же именем. */
    var BENEFITS =
  ...
  // Автопоказа нет — только по клику, как у «Чем полезен сервис».
  (function ownerHelpTrigger() {
    ...
    link.addEventListener('click', function () {
      // «Удобно» на последнем слайде — по макету (2690:128160), а не «Готово» ДС.
      openPromoModal({ layout: 'window', finishLabel: 'Удобно', originEl: link, slides: slides });
    });
  })();

...
| `.chip-m` | 44 ✔ | 8 ✔ | `#3686ff` ✔ | `#e6f0ff` ✔ |

То есть `.chip-s` попал в прод-цвета, но не в радиус; `.chip` — наоборот. 🟡
`.chip-m` совпадает с продом по геометрии и цветам, **кроме текста**: у нас Body1 16/24,
в снимке прода 14/20. Расхождение осознанное — в макетах компонент `Chip` (Figma 61:23298)
подписан Body1, и рядом с полем ввода той же высоты 14/20 читается мельче (правка Романа
2026-09-21, отчёт о конкурентах, секция «Дайте рекомендацию по цене»). Если пересниматься
с прода — сверить, не поменялся ли там сам `chip-button`, прежде чем откатывать.

...
var AppFilters = (function () {
  ...
  var PERIOD_DAYS = { 'за 7 дней': 7, 'за 14 дней': 14, 'за 30 дней': 30, 'за 3 месяца': 90 };
  ...
  var LIST = [
    ...
    /* Этаж — третьим, сразу за «До метро» (макет 2207:95963, правка Романа 2026-09-04).
       До этого он стоял восьмым, после материала дома: так было на проде, снятом
       2026-08-24. Порядок ряда решает макет, а не прод — на нём и остановились. */
    ...
    /* Одиннадцать вариантов прода в его же порядке. Было семь.
       ⚠️ Подпись вернулась к макетной — «Тип дома» (2207:95963, правка Романа
       2026-09-04). 2026-08-24 её меняли в обратную сторону, на продовую «Материал
       дома», потому что макеты тогда были старше прода. Теперь макет свежее. */
    { key: 'building', label: 'Тип дома', type: 'chips', title: 'Тип дома',
    ...
    /* Этажность дома (макет 2207:95963, 2026-09-04) — новый фильтр, на проде его нет.
       Значение лежит в самой записи (`floors` = `building.floorsCount` у ЦИАНа), а не
       разбирается из описания, как «Этаж»: там оно есть у всех 97 объявлений набора,
       разброс 4…47 при медиане 9. */
    { key: 'floorsTotal', label: 'Этажей в доме', type: 'range', title: 'Этажей в доме', unit: '',
      chip: function (v) { return rangeLabel(v, 'этажей'); },
      match: function (c, v) { return inRange(c.floors, v); } },

    /* Сроки — таблицей рядом с фильтром: их читает и `match`, и (через подписи)
       раскладка снятых в `mock-scenario.js`. Три месяца считаем 90 днями. */
    /* Только на архиве и первым в ряду. Наша доработка: на проде архив выбирается
       ...
       открывается со значением. Отсюда же и то, что подпись кнопки всегда несёт
       значение, и что «Сбросить» у него возвращает к базовому, а не гасит.

       ⚠️ Базовое значение — «за 3 месяца» (Роман, 2026-09-04, вместе с новым рядом
       фильтров); было «за 7 дней». Вариант добавлен четвёртым, три прежних остались:
       агент сужает от квартала к неделе, а не наоборот. На нашем наборе «за 3 месяца»
       показывает весь архив — то есть вкладка открывается полной, и фильтр работает
       на сужение.
    ...
    { key: 'period', label: 'за 3 месяца', type: 'chips', title: 'Архивные',
      archiveOnly: true, single: true, always: true, start: ['за 3 месяца'],
      options: ['за 7 дней', 'за 14 дней', 'за 30 дней', 'за 3 месяца'],
  ...
  /* Увидит ли агент это объявление в «Активных», не тронув ни одного фильтра.
   *

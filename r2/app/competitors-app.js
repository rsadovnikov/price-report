...
(function () {
  ...
  function openFilter(f) {
    ...
    /* «Сбросить» — справа в шапке, у ЛЮБОГО фильтра, `always` в том числе
       (Роман, 2026-09-01). Одно место и один вид на все девять; разница только в том,
       КУДА он сбрасывает: обычный фильтр гаснет, `always` возвращается к базовой
       настройке — 2 км, 15 минут, «за 7 дней». Гасить `always` нельзя: пустого
       состояния у него нет, и кнопка в ряду осталась бы без значения.

       Относится к ПРИМЕНЁННОМУ фильтру, а не к черновику выбора (Роман, 2026-08-22):
       пока не нажали «Применить», сбрасывать нечего — незаконченный выбор отменяется
       крестиком. Поэтому действие ставится один раз, при открытии, и по ходу выбора
       не меняется. Отсюда же и условие показа: у обычного — когда фильтр применён,
       у `always` — когда значение уехало от базового. Совпало с базовым — сбрасывать
       нечего, и кнопки нет: мёртвая кнопка в шапке хуже её отсутствия.

       Сам сброс — движение в один тап: правит состояние, обновляет выдачу и закрывает
       шторку (Роман, 2026-08-22). «Применить» после него не нужно — подтверждать
       нечего. */
    var изменён = f.always
      ? JSON.stringify(state[f.key]) !== JSON.stringify(f.start)
      : isFilled(f, state[f.key]);
    var reset = изменён
      ? { title: 'Сбросить', onClick: function () {
            if (f.always) state[f.key] = AppFilters.start(f);
  ...
  function resetFilters() {
    /* Остаются `always` — радиус, до метро и период: пустого состояния нет. По флагу,
       а не по имени: следующий такой фильтр иначе тихо исчезнет из ряда. */
    var keep = {};
    FILTERS.forEach(function (f) {
      if (f.always) keep[f.key] = state[f.key] != null ? state[f.key] : AppFilters.start(f);
    });
    state = keep;
    renderFresh();
  }
  ...
  /* «Отслеживается» — ПРИЗНАК объекта, а не принадлежность к вкладке. Раньше три
     ...
     Вкладка «Отслеживаемые» показывает содержимое множества, две другие — свои
  ...
  var marks = AppUpdates.allocate(ALL_COMPETITORS, n, u, params.get('desc') || '');
  ...
  /* Вкладку можно открыть сразу нужной: строка «Новый возможный конкурент» на
     обзоре ведёт в «Активные» — там кандидат и лежит, среди отслеживаемых его нет. */
  ...
  function render() {
    ...
    /* На «Отслеживаемых» лента не режется и хвоста не имеет: это не выдача, а выбор
  ...
  /* Переключение признака. Список НЕ перерисовываем — меняем кнопку на месте:
     иначе карточка мигает, а на вкладке «Отслеживаемые» ещё и уезжает под курсором. */
  ...
  listEl.addEventListener('click', function (e) {
    ...
    /* Корзина на вкладке «Отслеживаемые»: там показано само множество, поэтому
